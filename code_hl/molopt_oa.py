...
same as molopt.py:
  ...
  1. The starter reasons over a molecule/HOMO-LUMO-gap list and may call
     chemistry tools (grow_cycle, replace_groups, make_random_list, related,
     calculate_HL_gap, calculate_SAS_and_NP).
...
# --- Path setup (must run before importing the helpers) ----------------------
# This script now lives in code_hl/ alongside its helpers (MolPropOp,
# hl_gap_module, mock_tools), so they're importable as siblings -- no sys.path
# change needed. _ROOT is the repo root (parent of code_hl/), for data files
# that live there.
...
from hl_gap_module import (  # noqa: F401
    calculate_HL_gap,
...
ADVERSARY_INSTRUCTIONS = '''
    ...
    of novel molecules it has designed to have a small HOMO-LUMO gap (computed with the
    GFN2-xTB method). The proposal will include reasoning as to why the model thinks those
    molecules will have a small gap, and estimated gaps for each molecule. Your task is to
    analyze the proposal and find any flaws in the reasoning or estimation of the gaps. You
    should then suggest modifications to the proposed molecules that would make their gaps
    smaller, and provide reasoning for why those modifications would help.
    ...
    - grow_cycle: starts with a molecule SMILES and adds substituents to it, scores them, and returns
                  a list of molecules and gaps.

    - replace_groups: starts with a molecule SMILES and replaces specific groups in it with new groups,
                      returning a list of new molecules and gaps.
    ...
    - calculate_HL_gap: this tool calculates the HOMO-LUMO gap of a single molecule with GFN2-xTB
                and returns the gap in eV together with the frontier-orbital character.
...
- Ascertain any features of the molecules that contribute to a small gap. For example, if,
...
molecules and their HOMO-LUMO gaps, you have tools you can call to generate new
molecules and get their HOMO-LUMO gaps. You can use these tools as many times as you want
...
- grow_cycle: starts with a molecule SMILES and adds substituents to it, scores them, and returns
              a list of molecules and gaps. You can use this tool to further explore modifications
...
- replace_groups: starts with a molecule SMILES and replaces specific groups in it with new groups, returning a list of new
                  molecules and gaps. This tool allows you to test specific hypotheses about how replacing certain
                  groups in a molecule might affect the HOMO-LUMO gap. You can specify which groups to replace and
...
- make_random_list: this tool generates a list of substituents of specified length (num_items). It draws from the predefined lists:
                    ...
                    Use this tool when you want to get a broad sense of how different modifications affect the gap,
...
## If you have identified good potential hits, evaluate their synthetic accessibility with the
calculate_SAS_and_NP tool and use that information to further refine your proposals, keeping in mind
that you want to propose molecules that not only have small gaps but are also synthetically
accessible and chemically reasonable.
...
_TOOL_DEFS = [
    ('grow_cycle',
     'Add substituents to free carbons of a molecule, score the results with GFN2-xTB, and return '
     'a list of (molecule SMILES, HOMO-LUMO gap) pairs. Use it to explore modifications to a '
     'promising molecule. substituents may be a list of SMILES fragments or one of the predefined '
     'sets: e_withdraw, e_donate, withdraw_with_linkers, donate_with_linkers.',
     {'type': 'object',
      'properties': {
          'best_smiles': {'type': 'string', 'description': 'SMILES of the molecule to grow substituents onto.'},
          'best_score': {'type': 'number', 'description': 'Known/estimated HOMO-LUMO gap of best_smiles, in eV (optional).'},
    ...
    ('replace_groups',
     'Replace existing substituents in a molecule with new ones, score the results with GFN2-xTB, '
     'and return a list of (molecule SMILES, HOMO-LUMO gap) pairs. Use it to test hypotheses about '
     'swapping specific groups. substituents_to_replace / new_substituents may be lists of SMILES '
     'fragments or predefined set names: e_withdraw, e_donate, withdraw_with_linkers, donate_with_linkers.',
     {'type': 'object',
      'properties': {
          'orig_smiles': {'type': 'string', 'description': 'SMILES of the molecule to modify.'},
          'best_score': {'type': 'number', 'description': 'Known/estimated HOMO-LUMO gap of orig_smiles, in eV (optional).'},
    ...
    ('calculate_HL_gap',
     'Calculate the HOMO-LUMO gap of a single molecule with GFN2-xTB and return the gap in eV. '
     'Use only on a molecule already deemed to have a small gap and good synthetic accessibility, to '
     'confirm its frontier-orbital structure before proposing analogues around it.',
     {'type': 'object',
      'properties': {
    ...
    ('calculate_SAS_and_NP',
     ...
     'Call for promising molecules with small gaps and good synthetic accessibility.',
     {'type': 'object',
      'properties': {
          'smiles_list': {'type': 'array', 'items': {'type': 'string'},
            ...
            if tool_calls and tool_rounds < max_tool_calls:
                ...
                if tool_calls:
                    ...
                    messages.append({'role': 'user', 'content':
                        ...
                        "estimated gaps, and your reasoning. Do NOT say 'Done' — "
...
def run_session(args) -> str:
    ...
    results_path = os.path.join(args.results_dir, f"{safe}_{args.method}_{timestamp}.md")
    ...
    with open(results_path, 'w') as f:
        ...
        f.write(f'# method: {args.method} | starter: {starter_label} (tools) '
    ...
    if args.resume:
        ...
        # Initial prompt: the molecule / gap list for this method.
    ...
    while not is_done(last) and turn < args.max_turns:
        ...
        if last and last.strip():
            ...
            adv = ("Your last response had no text. Summarize your best proposed "
                   "molecules, their estimated gaps, and your reasoning. "
...
def self_test() -> None:
    ...
    print("\n-- calculate_HL_gap('c1ccc(O)cc1') --")
    print(calculate_HL_gap('c1ccc(O)cc1'))
...
Examples:
  ...
  python3 molopt_oa.py --method GFN2-xTB
...
""",
    ...
    p.add_argument('--method', default='GFN2-xTB',
                   help='Gap method: the xTB Hamiltonian passed to tblite (default: '
                        'GFN2-xTB; GFN1-xTB and IPEA1-xTB also available).')
    ...
    p.add_argument('--context-file', default=os.path.join(_HERE, 'adversarial_set.md'),
                   help='Initial molecule/gap list (default: code_hl/adversarial_set.md). '
    ...
    p.add_argument('--mock-tools', action='store_true',
                   help='Replace the three scoring-dependent tools (grow_cycle, replace_groups, '
                        'calculate_HL_gap) with an instant synthetic gap, skipping real '
                        'GFN2-xTB calculations. For smoke-testing the LLM/tool-calling and '
                        'adversary wiring in seconds instead of minutes -- not for real runs.')
    return p
