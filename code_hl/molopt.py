...
Refactor of Ollama_MolOpt.ipynb. Runs the same headless loop locally:
  1. An Ollama main model reasons over a molecule/HOMO-LUMO-gap list and may
     call chemistry tools (grow_cycle, replace_groups, make_random_list,
     related, calculate_HL_gap, calculate_SAS_and_NP).
...
# This script now lives in code_hl/ alongside its helpers (MolPropOp, hl_gap_module,
...
# hl_gap_module needs no numpy shim, but we keep the import order explicit
# of import order.
import numpy as np
...
def run_session(args) -> str:
    ...
    results_path = os.path.join(args.results_dir, f"{safe_model}_{args.method}_{timestamp}.md")
    ...
    with open(results_path, 'w') as f:
        ...
        f.write(f'# method: {args.method} | main model: {args.model} (think={think}) '
...
Examples:
  ...
  python3 molopt.py --method GFN2-xTB
