...
Adversary sets (each is one (proposer, adversary) pairing):
  ...
  anthropic_vs_anthropic molopt_oa.py --start anthropic --adversary anthropic (Anthropic
                          proposes+tools, Anthropic self-critiques -- same self-critique
                          ablation pattern as openai_vs_openai / gemini_vs_gemini)
  ollama_vs_ollama      molopt.py   --adversary ollama (Ollama proposes+tools, same Ollama
                          model self-critiques via a second stateless client -- same
                          self-critique ablation pattern as the *_vs_* sets above; needed
                          new tooling since molopt.py's --adversary only supported
                          openai/anthropic before)
...
Env: the venv MUST be activated (source fao-env/bin/activate) so the tblite library loads
(requires the activated fao-env, not fao-env/bin/python alone). API keys are inherited from
the environment (source ~/.zshrc first).
...
def build_configs(models):
    ...
    return {
        ...
        'ollama_vs_ollama': {
            'script': 'molopt.py',
            'args': ['--model', om, '--adversary', 'ollama'],
        },
        'gemini_vs_openai': {
            'script': 'molopt_oa.py',
        ...
        'anthropic_vs_anthropic': {
            'script': 'molopt_oa.py',
            'args': ['--start', 'anthropic', '--adversary', 'anthropic', '--anthropic-model', ant],
        },
    }
...
def build_argv(cfg, *, repdir, method, max_turns, max_tool_calls, quiet, mock_tools):
...
def main(argv=None) -> int:
    ...
    p.add_argument('--sets',
                   ...
                   help='Comma-separated subset of adversary sets to run (default: the 4 '
                        ...
                        'anthropic_vs_anthropic, ollama_vs_ollama.')
    p.add_argument('--method', default='GFN2-xTB', help='Gap method passed to tblite (default: GFN2-xTB).')
    ...
    p.add_argument('--mock-tools', action='store_true',
                   help='Pass --mock-tools through to every run: replaces the scoring-dependent '
                        'tools with an instant synthetic gap, skipping real GFN2-xTB '
                        'calculations. For smoke-testing a set/wiring change in seconds-per-rep '
                        'instead of minutes -- not for real runs.')
    args = p.parse_args(argv)

    # --- Env guard: tblite's native library must be importable (fao-env active). ---
    try:
        import tblite.interface  # noqa: F401
    except ImportError as err:
        print(f"ERROR: tblite not importable ({err}). Activate the venv first:\n"
              "  source fao-env/bin/activate\n"
              "(the GFN2-xTB gap calculations run through the tblite library in fao-env;\n"
              " fao-env/bin/python alone is not enough.)",
              file=sys.stderr)
        return 2
    ...
    batch_id = args.batch_id or f"hl_{args.method}_{time.strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    for label in selected:
        ...
        for rep in range(1, args.replicates + 1):
            ...
            argv = build_argv(cfg, repdir=repdir, method=args.method,
            ...
            if not args.force:
                ...
                if st is not None:
                    ...
                    _upsert(manifest, batch_id, label, rep, cfg['script'], args.method,
...
def _upsert(manifest, batch_id, label, rep, script, method, status,
    ...
    entry.update({'batch_id': batch_id, 'script': script, 'method': method,
